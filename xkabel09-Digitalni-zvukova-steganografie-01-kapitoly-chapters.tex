...
\cite{Swanson1998}. Zvuk je ve skutečném světě spojitý -- má nekonečný počet
hodnot -- a~je tedy definován všude. Aby bylo možné se zvukem pracovat
digitálně, je nutné převést jej do diskrétní formy, která má konečný počet
hodnot. Toho docílíme vzorkováním zvuku na nějaké frekvenci
\cite{Cernocky2021}. Vzorkování probíhá tak, že se každá sekunda spojitého
zvuku rozdělí na tolik bodů, kolik udává vzorkovací frekvence. V~každém bodě se
zjistí amplituda ve stejném bodě spojitého zvuku a~ta se stane jeho hodnotou.
Rozdíl mezi spojitým a~navzorkovaným signálem je vidět na
obrázku~\ref{pic:continuous-vs-discrete-signal}. Nejčastěji používané
vzorkovací frekvence jsou 44\,100\,Hz a~48\,000\,Hz.
    ...
    \begin{subfigure}[b]{0.5\linewidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[%
                ...
                xlabel = {$t$},
                ylabel = {$x(t)$}]
                \addplot[black,thick] {sin(2*180*x)};
            \end{axis}
        \end{tikzpicture}
    \end{subfigure}%
    \begin{subfigure}[b]{0.5\linewidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[%
                ...
                xlabel = {$n$},
                ylabel = {$x[n]$}]
                \addplot+[ycomb,black,thick] {sin(2*180*x/16)};
            \end{axis}
        \end{tikzpicture}
    \end{subfigure}
    \caption{Spojitý signál (vlevo) a~diskrétní signál (vpravo) vzorkovaný na
    frekvenci 16\,Hz}
    \label{pic:continuous-vs-discrete-signal}
\end{figure}

\subsection*{Formáty digitálního zvuku}

\todo{popsat formaty a popsat WAV}
\blindtext
