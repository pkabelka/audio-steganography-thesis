...
\begin{itemize}
    ...
    \item \textit{audio-steganography-algorithms}\footnote{\url{https://github.com/ktekeli/audio-steganography-algorithms}}
        ...
        rozprostřeného spektra (viz podsekce~\ref{sub:dsss}), ukrývání pomocí ozvěny
        (viz podsekce~\ref{sub:echo-hiding}), nejméně významného bitu
        (viz podsekce~\ref{sub:lsb}) a~fázového kódování (viz podsekce~\ref{sub:phase-coding}).
...
rozprostřeného spektra (viz podsekce~\ref{sub:dsss}). Aby bylo kódování robustnější,
...
viz kapitola~\ref{cha:statistical-functions}). Použitím této optimalizační funkce je
...
viz příloha~\ref{cha:statistical-functions}) a~špičkového poměr signálu k~šumu (PSNR,
viz příloha~\ref{cha:statistical-functions}) pro jednotlivé testované parametry.
...
Tato varianta má ze všech čtyř nejvyšší hodnoty SNR a~PSNR a~hodnoty BER
...
(viz obrázek~\ref{pic:modifications-mean-values-echo-bipolar-bf}).
...
Pro lepší přehled výsledných hodnot nemodifikovaných stego signálů
z~automatického a~ručního testování obsahuje tabulka~\ref{tab:results-summary}
výsledné hodnoty SNR, PSNR a~kapacitu metod pro krycí signál vzorkovaný na
frekvenci~44,1\,kHz.
