...
Následující kapitoly této práce shrnou současný stav této oblasti, vysvětlí
cíle a~technické prostředky použité k~jejich dosažení a~zhodnotí dosažené
výsledky. V~kapitole~\ref{cha:existing-methods} \uv{Přehled existujících metod
digitální zvukové steganografie} jsou do hloubky popsány steganografické metody
vybrané pro implementaci a~některé další používané.
Kapitola~\ref{cha:library-design} popisuje proč byl vybrán programovací jazyk
Python a~použité knihovny. Je zde také popsána struktura balíčků pro Python
a~steganografické metody které byly vybrány pro implementaci včetně vlastní.
Kapitola~\ref{cha:implementation} je zaměřená na popis rozvržení kódu
a~implementaci steganografických metod. Na konci je popsán způsob vyhodnocení
kvality metod, výsledky testování a~srovnání metod. Poslední
kapitola~\ref{cha:conclusion} obsahuje shrnutí výsledků práce.
