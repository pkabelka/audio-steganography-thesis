...
\noindent Po výpočtu cepstra segmentu je možné zjistit hodnotu zakódovaného bitu
...
\noindent Tato metoda má malou kapacitu a~vložené tóny je jednoduché detekovat, proto je
...
\noindent Pokud $X$ je kódovaná hodnota, $N$ je počet bitů kódované hodnoty a~$L$ je
...
\todo{popsat strukturu Python balicku}
\blindtext
...
\noindent Rovnice těchto funkcí jsou v~příloze~\ref{cha:statistical-functions}. Při
...
\noindent Nepostřehnutelnost je sice subjektivní vlastnost, ale je možné částečně
...
\noindent V~kořenovém balíčku se nacházejí moduly společné pro ostatní balíčky. Tyto
moduly jsou:
...
\noindent Nové fáze se zapíší do horních frekvencí, které se nachází v~polovině pole
...
\noindent Kód pro zápis nových fází ekvivalentní s~rovnicemi~\ref{eq:phase-new-angles} se
...
\noindent V~implementaci jsou amplitudy předpočítány pro všechny kombinace bitů. Výpočet
bude proveden efektivněji, protože knihovna NumPy použije na pozadí
optimalizované vektorové instrukce. Výpočet tak bude mnohem rychlejší než při
použití cyklu v~jazyce Python. Předpočítané amplitudy jsou poté vybrány na
základě kódovaného bitu a~násobí se s~příslušným tónem. Ukázka kódu pro výpočet
amplitud a~modulaci tónů se segmenty je vidět ve
