...
souborů, která bude s~ostatními více popsána dál
...
složku obsahující datové sady. Při načítání cest souborů z~datových sad jsou
ignorovány datové sady a~kategorie začínající \uv{.}. Všechny soubory které
...
přidávají vyhodnocované aspekty. První cyklus testuje různé délky zpráv.
V~dalším cyklu se každá zpráva kóduje do souboru zvolenými metodami. Každá
metoda má také různé parametry které ovlivňují míru nepostřehnutelnosti,
robustnosti a~kapacity. Proto musí být tyto parametry otestovány navzájem
s~ostatními pro nalezení optimálních hodnot. Po zakódování zprávy jsou
aplikovány modifikace a~vyhodnocení vlastností statistickými funkcemi popsanými
v~sekci~\ref{sub:method-property-verification}. Výsledky pro vyhodnocený soubor
jsou z~cyklů postupně sesbírány do instance třídy \texttt{DataFrame} z~knihovny
\textit{pandas} a~zapsány ve formátu \textit{CSV} do výstupní složky.
...
kódovány pomocí adaptivní diferenciální pulzně-kódové modulace, pro kterou
nebyl implementovaný program navržen.
...
Testované délky jsou 32\,bitů, 128\,bitů, 512\,bitů a~v~rozšířeném režimu
i~256\,bitů.
